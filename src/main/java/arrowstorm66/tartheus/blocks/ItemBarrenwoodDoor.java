...
public class ItemBarrenwoodDoor extends BasicItem implements ModelRegistry {
	protected String name;

	public ItemBarrenwoodDoor(final String name) {
		super(name);
		this.name = name;
	...
	public EnumActionResult onItemUse(final EntityPlayer player, final World world, BlockPos pos, final EnumHand hand,
		...
		if (player.canPlayerEdit(pos, facing, stack) && MBlocks.BARRENWOOD_DOOR.canPlaceBlockAt(world, pos)) {
			...
			final boolean flag = (i < 0 && hitZ < 0.5f) || (i > 0 && hitZ > 0.5f) || (j < 0 && hitX > 0.5f)
					|| (j > 0 && hitX < 0.5f);
			ItemDoor.placeDoor(world, pos, enumfacing, MBlocks.BARRENWOOD_DOOR, flag);
			final SoundType soundtype = MBlocks.BARRENWOOD_DOOR.getSoundType();
