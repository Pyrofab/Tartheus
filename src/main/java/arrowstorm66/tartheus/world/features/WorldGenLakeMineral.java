...
public class WorldGenLakeMineral extends WorldGenerator {
	private final Block block;
	/** The maximum radius used when generating a patch of blocks. */
	private final int radius;

	public WorldGenLakeMineral(Block p_i45462_1_, int p_i45462_2_) {
				...
				for (int l = position.getZ() - i; l <= position.getZ() + i; ++l) {
					...
					if (i1 * i1 + j1 * j1 <= i * i) {
						for (int k1 = position.getY() - 2; k1 <= position.getY() + 2; ++k1) {
							...
							if (block == MBlocks.TARTHEUS_DIRT || block == MBlocks.TARTHEUS_GRASS) {
