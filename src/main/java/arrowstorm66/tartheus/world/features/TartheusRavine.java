...
public class TartheusRavine extends MapGenBase {
	...
	protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop) {
		net.minecraft.world.biome.Biome biome = world.getBiome(new BlockPos(x + chunkX * 16, 0, z + chunkZ * 16));
		IBlockState state = data.getBlockState(x, y, z);
		IBlockState top = isExceptionBiome(biome) ? MBlocks.TARTHEUS_GRASS.getDefaultState() : biome.topBlock;
		IBlockState filler = isExceptionBiome(biome) ? MBlocks.TARTHEUS_DIRT.getDefaultState() : biome.fillerBlock;

		if (state.getBlock() == MBlocks.STONE_THIRD_STRATUM || state.getBlock() == MBlocks.STONE_SECOND_STRATUM
				|| state.getBlock() == MBlocks.STONE_FIRST_STRATUM || state.getBlock() == top.getBlock()
