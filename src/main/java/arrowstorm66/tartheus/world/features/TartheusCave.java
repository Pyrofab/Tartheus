...
public class TartheusCave extends MapGenBase {
	...
	protected boolean canReplaceBlock(final IBlockState p_175793_1_, final IBlockState p_175793_2_) {
		return this.isStone(p_175793_1_) || this.isSurface(p_175793_1_);
	}

	private boolean isStone(final IBlockState state) {
		return state.getBlock() == MBlocks.STONE_THIRD_STRATUM || state.getBlock() == MBlocks.STONE_SECOND_STRATUM
				|| state.getBlock() == MBlocks.STONE_FIRST_STRATUM || state.getBlock() == MBlocks.MARROWSTONE
				|| state.getBlock() == MBlocks.TARTHEUS_SANDSTONE || state.getBlock() == MBlocks.TERRACOTTA
				|| state.getBlock() == MBlocks.TERRACOTTA_CRIMSON || state.getBlock() == MBlocks.TERRACOTTA_GRAY
				|| state.getBlock() == MBlocks.TERRACOTTA_HAZEL || state.getBlock() == MBlocks.TERRACOTTA_PALE
				|| state.getBlock() == MBlocks.TERRACOTTA_TANGERINE;
	}

	private boolean isSurface(final IBlockState state) {
		return state.getBlock() == MBlocks.TARTHEUS_SAND || state.getBlock() == MBlocks.HELL_SAND
				|| state.getBlock() == MBlocks.TARTHEUS_GRASS || state.getBlock() == MBlocks.BONE_PILE
				|| state.getBlock() == MBlocks.BONE_MUSH;
	}
	...
	private boolean isExceptionBiome(net.minecraft.world.biome.Biome biome) {
		if (biome == MBiomes.DESERT)
			return true;
		if (biome == MBiomes.DESERT_HILLS)
			return true;
		if (biome == MBiomes.BADLANDS)
			return true;
	...
	private boolean isTopBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ) {
		...
		return (isExceptionBiome(biome) ? state.getBlock() == MBlocks.TARTHEUS_GRASS
